...
rm -rf "$DEB_DIR/data"
mkdir -p "$DEB_DIR/$PFX/bin" "$DEB_DIR/$PFX/share/termux-vapt" "$DEB_DIR/$PFX/share/doc/termux-vapt" "$DEB_DIR/$PFX/share/man/man1" "$DEB_DIR/$PFX/share/bash-completion/completions"
cp -a src/termux_vapt/__init__.py src/termux_vapt/auth.py \
   ...
   src/termux_vapt/templates/ "$DEB_DIR/$PFX/share/termux-vapt/"

echo "[2/4] Copying wrapper..."
cp src/termux-vapt "$DEB_DIR/$PFX/bin/"
chmod 755 "$DEB_DIR/$PFX/bin/termux-vapt"
